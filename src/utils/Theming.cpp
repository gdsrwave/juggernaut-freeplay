...
std::string handleRawBlock(std::string addBlockLine, OMType omType) {
    ...
    while (start < addBlockLine.size()) {
        ...
        // in 2.2, block/"GameObject"s' data will always have attributes 155 and 156 applied to them. these are automatically
        // generated by GD using a currently unknown technique, and when they are mismatched, issues such as block flickering, 
        // incorrectly set colors, incorrectly changing colors etc. will appear when a level is played. because of this, JFP
        // will strip these values. if you face issues with removing these values, I unfortunately don't have a great answer;
        // all I can do is suggest you create theme segments in 2.1 instead. thanks for understanding. -M
