...
class $modify(LBGenerateLevelLayer, LevelBrowserLayer) {
    ...
    // waveman button callback -- return type here must be void and parameter
    // must be CCObject*
    void onGenButton(CCObject*) {
        ...
        if (jfpResult.isErr()) return FLAlertLayer::create(
            "Import Error", jfpResult.unwrapErr(), "Sure thing...")->show();

        // Insert level object into local list
        LocalLevelManager::get()->m_localLevels->insertObject(
            jfpResult.unwrap(), 0);

        // create new scene
        auto newScene = CCScene::create();
        auto newLayer = LevelBrowserLayer::create(
