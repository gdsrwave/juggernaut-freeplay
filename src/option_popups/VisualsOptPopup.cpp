...
bool VisualsOptPopup::setup(std::string const& value) {
        ...
        auto colorModePicSpr = CCSprite::createWithSpriteFrameName(
            fmt::format("jfp_cm_{}.png"_spr, m_colorModeIndex).c_str()
        );
        colorModePicSpr->setScale(180.f / colorModePicSpr->getContentSize().height);
        //colorModePicSpr->setColor({255, 136, 237});
        ...
        colorModeMenu->addChild(colorModePicSpr);
        ...
        auto bgTexturePicSpr = CCSprite::create(
            fmt::format("game_bg_{:02}_001.png", m_bgTextureIndex + 1).c_str()
        );
        bgTexturePicSpr->setScale(180.f / bgTexturePicSpr->getContentSize().height);
        bgTexturePicSpr->setColor({29, 120, 86});
        ...
        m_bgTextureSelected = CCLabelBMFont::create(
            fmt::format("Background {:02}", m_bgTextureIndex + 1).c_str(), "bigFont.fnt"
        );
        ...
        bgTextureMenu->addChild(bgTexturePicSpr);
...
void VisualsOptPopup::save(CCObject*) {
    ...
    mod->setSavedValue<uint32_t>(
        "opt-0-mark-interval",
        numFromString<int32_t>(m_markIntInput->getString()).unwrapOr(0)
    );
    ...
    mod->setSavedValue<bool>("opt-0-color-mode", m_colorModeIndex);
    mod->setSavedValue<bool>("opt-0-background-texture", m_bgTextureIndex);
...
void VisualsOptPopup::onEnumDecrease(CCObject* object) {
    ...
    //     m_ssizeIndex -= 1;
    //     if (m_ssizeIndex < 0) m_ssizeIndex = m_ssizeIndexLen - 1;
    //     labelText = JFPGen::StartingSizeLabel.at(m_ssizeIndex);
    // } else if (lbl->getID() == "jfpopt-transition-type") {
    //     m_tTypeIndex -= 1;
    //     if (m_tTypeIndex < 0) m_tTypeIndex = m_tTypeIndexLen - 1;
